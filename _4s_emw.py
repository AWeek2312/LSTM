...
def df_plot_col_before(df, column_name, num):
    ...
    plt.title(f'50°倾角4s停顿-第{num}趟数据平滑前-{column_name}随时间变化曲线')  # 设置标题
    plt.grid(True)  # 显示网格
    plt.savefig(f"C:\\Users\\Aweek\\Desktop\\数据平滑\\4s\\50度\\第{num}趟\\平滑前图像\\{column_name}.png")
    plt.show()  # 显示图形
...
def df_plot_col_after(df, column_name, num):
    ...
    plt.xlabel('时间步')  # 设置x轴标签
    plt.ylabel(column_name)  # 设置y轴标签
    plt.title(f'50°倾角4s停顿-第{num}趟数据平滑后-{column_name}随时间变化曲线')  # 设置标题
    plt.grid(True)  # 显示网格
    plt.savefig(f"C:\\Users\\Aweek\\Desktop\\数据平滑\\4s\\50度\\第{num}趟\\平滑后图像\\{column_name}.png")
    plt.show()  # 显示图形
